package com.zwyl.myhomework.viewstate.treelist;
...
public class TreeHelper {
...

    {
        ...
        for(T t : datas) {
            ...
            for(Field f : declaredFields) {
                ...
                if(f.getAnnotation(TreeNodeClickid.class) != null) {
                    f.setAccessible(true);
                    clickId = (String) f.get(t);
                }
                if(id != - 1 && pId != - 1 && label != null && clickId != null) {
                    break;
                }
            }
            node = new Node(id, pId, label, clickId);
            nodes.add(node);
        }
    ...
    private static void setNodeIcon(Node node) {
        if(node.getChildren().size() > 0 && node.isExpand()) {
            node.setIcon(R.mipmap.arrowsdown);
        } else if(node.getChildren().size() > 0 && ! node.isExpand()) {
            node.setIcon(R.mipmap.arrowsright);
        } else
            node.setIcon(- 1);
