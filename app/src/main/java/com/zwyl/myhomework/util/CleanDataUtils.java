...
public class CleanDataUtils {
    ...
    public static String getCacheSize(Context context) {
        ...
        if (dataDataCache.exists()) {
            result += getFolderSize1(dataDataCache);
        }
        ...
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            ...
            if (null != SDCache && SDCache.exists()) {
                result += getFolderSize1(new File(SDCache.getAbsolutePath()));
            }
        }
    ...
    // 获取某个文件夹下面所有文件大小
    private static long getFolderSize1(File file) {
        long size = 0;
        try {
            File[] fileList = file.listFiles();
            for (File f : fileList) {
                if (f.isDirectory()) {
                    size += getFolderSize1(f);
                } else {
                    size += f.length();
                }
            }
